// Canonical catalog seed data. This is the data that used to live inline in
// src/data/store.ts. It is loaded into the database once, on first server start
// (see src/server/seed.ts + instrumentation.ts). Editing here + wiping the DB
// re-seeds; day-to-day edits happen through the /admin panel against the DB.

import { getProductImageUrl } from "@/lib/product-images";
import mangalsutraImg from "@/assets/jewelary-img/Mediaa.jpg";
import mangalsutraLeafImg from "@/assets/jewelary-img/d.jpg";
import diamondNecklaceImg from "@/assets/jewelary-img/Media.jpg";
import heartDropEarringsImg from "@/assets/jewelary-img/y.jpg";
import pinkHaloRingImg from "@/assets/jewelary-img/rr.jpg";
import rubyBangleImg from "@/assets/jewelary-img/w.jpg";

export interface SeedGemstone { type: string; caratWeight?: number; cut?: string; clarity?: string; count?: number; }
export interface SeedCertification { body: string; number?: string; }

export interface SeedBrand { id: string; slug: string; name: string; }
export interface SeedCategory { id: string; slug: string; name: string; description?: string; parentId?: string; position?: number; }
export interface SeedReview { id: string; productId: string; rating: number; author: string; body: string; }
...
export interface SeedProduct {
  ...
  gemstones?: SeedGemstone[]; certifications?: SeedCertification[];
  collectionLine?: string; gender?: string; tags?: string[];
}

export interface SeedCollection {
  id: string; slug: string; title: string; intro?: string;
  filter: { categorySlug?: string; brandSlug?: string; color?: string; maxPriceInPaise?: number };
}
...
const rawProducts: SeedProduct[] = [
  {
    ...
    rating: 4.9, reviewCount: 86, sizes: ["10","11","12","13","14","15","16"], tags: ["Best Seller"],
  },
  {
    ...
    rating: 4.8, reviewCount: 64, sizes: ["10","11","12","13","14","15","16"], tags: ["Limited Stock", "Trending"],
  },
  {
  ...
  },
  {
    ...
    certifications: [{ body: "BIS" }],
    rating: 4.6, reviewCount: 73, sizes: ["12","13","14","15","16","17","18"], tags: ["New Arrival"],
  },
  {
...
// Resolve a display image where none is set (keyword-matched placeholder).
export const seedProducts: SeedProduct[] = rawProducts.map((p) => ({
  ...p,
